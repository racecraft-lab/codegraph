...
| MCP dogfooding (HEAD build) | `.mcp.json` | `.codex/config.toml` `[mcp_servers.codegraph]` — project-scoped MCP is official, trusted projects only ([mcp docs](https://developers.openai.com/codex/mcp)) | Both launch via **`scripts/mcp-dogfood.mjs`** — a cross-platform Node launcher (`command: "node"`, no POSIX `sh`, so Windows clones work) reached through a `node -e` walk-up locator that finds the checkout root from any cwd inside the repo; it applies the untracked `.envrc.local` to the daemon env whether or not values are `export`ed (spec worktrees fall back to the main checkout's copy via `git rev-parse --git-common-dir`), selects the first PATH runtime satisfying the package's Node engine range when the MCP host itself runs an unsupported Node, then spawns `node dist/bin/codegraph.js serve --mcp`. The checked-in Codex profile sets `CODEGRAPH_NO_DAEMON=1` and `CODEGRAPH_NO_WATCH=1`: Codex's workspace sandbox cannot create the POSIX election lease under `~/.codegraph`, and its watcher resources are not reliable, so run `node dist/bin/codegraph.js sync .` before a Codex dogfood session. Per clone: `npm run build && node dist/bin/codegraph.js init .`; Codex asks to trust the project on first run |
...
| Prompt front-load hook | `.claude/settings.json` `UserPromptSubmit` → `dist/bin/codegraph.js prompt-hook` (guarded: silently skipped until `dist/` is built; mirrors the product's `codegraph prompt-hook`) | Not wired — Codex has a `UserPromptSubmit` hook event, but the product's prompt-hook speaks Claude's hook stdin/stdout contract; the AGENTS.md retrieval policy carries the function | Candidate future parity item |
| LSP precision dogfood (SPEC-008) | Root `codegraph.json` (`lsp.enabled: true`, plus `lsp.caps.fullIndexWorkItemsPerLanguage` raised so the pass verifies this repo's full candidate set instead of stopping at the default 10k per-language work cap) — host-agnostic | Same file; any index/sync from either host runs the pass | Needs a language server on PATH (e.g. `typescript-language-server`); degrades gracefully with reason codes when absent |
...
| MCP tool pre-approval | Tracked `.claude/settings.json` ships `mcp__codegraph__*` allow + `enableAllProjectMcpServers` (mirrors the product installer's `autoAllow`; Claude ignores repo-tracked approvals in untrusted folders) | Not needed — Codex does not prompt per MCP tool call | Functional parity once the folder is trusted |
...
- MCP launch behavior changes land ONCE in `scripts/mcp-dogfood.mjs` — both hosts'
  configs point at it (via the same `node -e` walk-up locator; keep the two
  config copies of that one-liner identical). Spec-worktree bootstrap (build + init + coverage check before
  agent work) is normative in the roadmap's § Dogfooding Protocol, rung 6.
