...
- Shipped agents follow the operator-owned tool-surface doctrine: no `tools:` allowlists (inherit the operator's full surface), built-in-only `disallowedTools` per each agent's tier decision; skills are user-invocable workflows over existing capabilities; no artifact restates the MCP `initialize` guidance (issue #529).
...
- Validation gates (binding, from SPEC-025): per-artifact retrieval A/B evidence before first release; retrieval-guardian review for any retrieval-affecting steering surface.
