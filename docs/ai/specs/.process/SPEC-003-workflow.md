...
- [x] `npm run eval` semantic cases: NOT RUNNABLE on this machine — the pre-existing eval framework requires `EVAL_CODEBASE` pointing at an already-indexed Elasticsearch clone (all 16 cases, incl. the 4 T028 hybrid-paraphrase cases, target ES-shaped symbols; no such corpus exists locally). Pre-existing limitation (predates SPEC-003, commit 13d3ff3), recorded in pr-review-packet.md known-gaps. FR-014 evidence stands on the deterministic CI gate instead: `hybrid-search.test.ts` 84/84 (hit-rate ≥ keyword, semantic-only strictly greater, byte-stability, p95 85.2ms vs 150ms) inside the green 2806/0/7 run. Evidence: `.process/integration-suite.md`
