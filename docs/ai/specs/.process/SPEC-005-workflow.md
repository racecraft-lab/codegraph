...
| IMPL-1 | daemon read transport (plan contradiction) | implement-executor evidence (file:line) → human gate | human ratified preferred option | additive structured read-only `codegraph/read` JSON-RPC on the daemon session dispatch; FR-002/Q1 upheld (all reads over the socket, one warm index); plan Constitution III/VI rows amended; retrieval-guardian now guards BOTH slice PRs | ratified, plan+tasks amended |
