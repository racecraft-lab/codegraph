...
| Implement | `/speckit-implement` | ✅ Complete | 2026-07-12. 53/53 tasks in 4 groups (Foundation; Slice-1 plan engine + refusals; Slice-2 apply ladder + MCP + guidance; Polish + gate). 4 dogfood/gate defects found + fixed same-session with TDD (D1 edge scoping, D2 refusal payloads, D3 LSP completeness degrade, D4 plan-time drift refusal). retrieval-guardian SHIP-WITH-NOTES 6/6; T048 A/B NO-REGRESSION; Docker Linux non-root 182/182; quickstart S1-A…F + S2-A…I PASS. Full suite 3110/0 (175 files). **G7 PASS** (7/7 checks, evidence re-run fresh by gate-validator). Slice markers checkpointed (`f911f95` slice-1, `acf1a60` slice-2); PR packet at `specs/010-graph-aware-rename/.process/slice-2-pr-packet.md`. |
...
- [x] `codegraph rename <target> <new-name>` (dry-run default) prints a plan: files, ranges, before/after previews, confidence per edit — for any indexed language.
- [x] LSP path used when a configured server covers the language; graph-reference path otherwise; the plan states which path produced each edit.
- [x] Ambiguous target → refusal listing every candidate (kind, file:line, selecting qualifier); zero writes.
- [x] `--apply` refuses when any edit is below `exact` unless `--include-heuristic`; refusal lists the gated edits.
- [x] `--apply` verifies every span against live bytes first; mismatch → whole apply aborts ("stale index — run codegraph sync"), zero writes.
- [x] Apply is atomic: workspace-root jail + .gitignore respect; pre-write snapshots; targeted re-sync of touched files; post-check asserts zero dangling references; post-check failure → full rollback + re-sync + report.
- [x] `codegraph_rename` MCP tool ships the same contract (dry-run default, explicit `apply: true`); expected refusals are success-shaped (never `isError`); `server-instructions.ts` updated.
- [x] Control-repo A/B shows no retrieval regression from the enlarged tool list (Sonnet floor, ≥2 runs/arm).
- [x] Self-repo dogfood UAT: rename a real symbol in this repository via the new capability, plan reviewed, applied, post-check green, then reverted (or kept if trivial) — evidence recorded in the UAT runbook.
- [x] macOS + Dockerized Linux suites green; Windows-sensitive assertions gated `it.runIf` and recorded as deferred.
...
| 3 - Slice 2: apply + MCP | T028–T048 | ✅ 2026-07-11 (`4751425`, `00de5f7`, `6a3cdbd`, `e5251fb`) | Apply ladder (T028–T042, 60 tests): jail (realpath-both-sides + scope-ignore → whole-plan success-shaped refusals), in-memory snapshots → temp-sibling atomic writes (descending edit order, byte preservation), resolution-complete sync with zero-shape discrimination (`filesChecked===0&&durationMs===0`), touched-file-scoped dual-assertion post-check, rollback + recovery dump (sole reachable isError = rollback-failed), CLI `--apply`/`--include-heuristic` exits {0,1,2,3,4}; all four apply terminals induced through the real pipeline; SC-010 count-stability proven; full suite 3082/0. MCP group (T043–T048): `codegraph_rename` ToolDefinition with own FR-028 annotation quadruplet, `DEFAULT_MCP_TOOLS` {explore, rename}, main-instance dispatch (never query-worker offload), CLI `--json` byte-parity via the shared canonical serializers (13 tests); server-instructions write-tool paragraph with anti-dilution pins (6 tests; `NO_ROOT_INDEX` variant deliberately untouched, artifact-grounded); full suite 3101/0 (175 files). retrieval-guardian: **SHIP-WITH-NOTES, 6/6 checks PASS** (explore primacy, steering byte-intact +14/−0, Principle-VI isError audit — the refactor engine carries exactly one impossible-by-construction throw, read-path neutrality, pin integrity, variant coherence); its 2 doc advisories fixed in `e5251fb`. **T048 A/B (express@4.21.2, baseline `4751425`, 2 runs/arm, sonnet/high, scrubbed env): NO-REGRESSION** — new arm 37–57s / Read 1–2 / Grep 0 vs baseline 45–55s / Read 1 / Grep 0; `codegraph_explore` the only codegraph tool called in all 4 runs; `codegraph_rename` provably exposed (tools 2 vs 1) yet never mis-picked on the rename-neutral task; $0.38–0.44/run; engine tree verified restored. Daemon-prewarm WARNs in all 4 arms ruled benign by by-type usage evidence (documented nested-session artifact). |
| 4 - Polish & cross-cutting | T049–T053 (+D3, +D4) | ✅ 2026-07-12 | T049 CHANGELOG apply+MCP entry + `.cursor/rules/codegraph.mdc` two-tool update (`cdc56d7`). T052 gating audit: **zero ungated platform-sensitive assertions** (symlink-jail ×2 + chmod/EACCES rollback-failed ×3 all `it.runIf`-gated; CRLF/BOM Buffer-level neutral by design; 188/188 macOS baseline proves the gated tests execute, not skip). Windows deferral note: apply/rollback end-to-end unvalidated on Windows — tracked follow-up, VM suspended (spec Assumptions / design-concept Q10). T051 Docker Linux (`node:22-bookworm`, `--rm --init`): root run 179/182 — the 3 fails are the chmod-0444 rollback-failure simulation bypassed by root DAC, deterministic ×2, documented environment artifact; **non-root (chown + USER node, uid 1000): 182/182**, the 3 target tests confirmed by name filter. T050 self-repo dogfood: refusal probes (ambiguous-target with candidates, target-not-found), plan↔diff scope conformance, revert + index stability (7,882 nodes / 31,664 edges byte-identical across mutate→revert) all PASS — and **found defect D3**: the LSP plan covered only the declaration file on this 381-file TS repo (tsserver project-load race; `deriveEdits` trusted any ok-status LSP result unconditionally), so `--apply` exited 0 with green touched-file-scoped post-check while tsc broke (TS2305) and 9 tests failed at runtime. **D3 remediated** under the FR-003a unusable-result taxonomy (line-198 overlap precedent; "graph-verified always"): graph edits now derived first on every rename, an ok LSP result must cover every span-verified graph-edit file else the whole rename degrades to graph with plan-surfaced `lspDegradation: incomplete-coverage`; RED→GREEN (111/111 plan suite; full suite 3103/0); spec.md/data-model.md/schema amended; tsserver project-load warm-up recorded as out-of-scope follow-up (safety must not depend on server timing). T050 post-fix validation re-run (ab7f287): **PASSED** — plan degraded to graph with `lspDegradation: incomplete-coverage` on both surfaces, covering all 3 reference files (9 edits, all-exact; leftoverMentions 1→11 reconciled exactly against the grep inventory — JSDoc `{@link}`/comment/string mentions correctly leftover-only per FR-013); apply rewrote all 3 files, post-check green; **tsc clean post-apply** (the original TS2305 gone) and refactor-apply 60/60 (the 9 TypeErrors gone); byte-clean revert, index stable (7,885/31,686 pre≡post), no residue. Note for the record: this symbol shape had no intra-file call site — the LSP run's extra snapshot.ts edits were JSDoc `{@link}` mentions, which the graph path deliberately leaves as leftovers (comments aren't graph references), so a completed graph-path rename leaves doc-comment mentions to the leftover FYI. **T053 gate**: build clean; full suite 3103/0 pre-D4; quickstart S2-A…I executed — 8 PASS, S2-C **FAIL exposed defect D4** (pre-existing index drift silently conflated with the FR-005 false-positive drop → silently-incomplete plan applies exit 0, contradicting spec.md:94/SC-002/SC-004; same silent-incompleteness class as D1/D3, third mechanism). **D4 remediated same-session** (spec-anchored, no design fork): plan-time index-freshness check over all candidate files (indexer's own size+mtime fast path + `hashContent` sha256; deleted/untracked = drift) → whole-plan `stale-span` refusal with sync guidance; false-positive drops in fresh files unchanged (SC-008 coupling pinned); FR-016 apply-window untouched; check ordered after the jail (refuse-before-read). 7 TDD tests (CRLF RED proved span checks are `\r`-tolerant — only file-level hashing satisfies the spec's CRLF clause; deleted-file RED exposed + fixed a latent uncaught ENOENT). **Re-gate: full suite 3110/0 (175 files), tsc + build clean; live CLI loop verified (drift → refusal exit 2 zero writes → `codegraph sync` → applies exit 0 post-check green).** spec.md FR-005 + Clarify Q&A amended with the freshness discriminator; PR packet `specs/010-graph-aware-rename/.process/slice-2-pr-packet.md` assembled (FR traceability, SC-001…010 map all Solid post-D4, known gaps = Windows deferral + tsserver warm-up + mid-write kill window). Observation for future UX polish (not a defect): `codegraph sync` lacks `--path`, so the stale-span remedy runs from the project cwd. |

### Post-Implementation Findings (steps #86–#89)

- **Doctor** (#86): skipped — extension not installed (Phase-0 disposition re-confirmed).
- **Verify Implementation** (#87, `speckit.verify.run`): **clean** — 0 CRITICAL/HIGH/MEDIUM; 29/29 FRs evidenced, 26/26 files, 197/197 scoped suite + tsc re-verified independently; 2 LOW (SC-009 runbook pending → step #93 by design; stale pre-Clarify checkbox → fixed `1b24b57`).
- **Verify Tasks phantom check** (#88, `speckit.verify-tasks.run`): **0 phantoms across 53 tasks** (46 verified / 2 procedural-skip / 4 evidence-class-weak-expected / 1 PARTIAL). The PARTIAL — T027's named Slice-1 PR packet never produced as a file — closed by assembling `specs/010-graph-aware-rename/.process/slice-1-pr-packet.md` (`79eb7fc`); comment-rot finding (post-check.ts stale STUB header) fixed `2fc8702`; report filed under `.process/`.
- **Code Review** (#89, full branch code diff): 1 BLOCKER + 2 MAJOR + 1 MINOR + 1 NIT — **all four fixable findings remediated same-session as the D5 batch** (TDD, RED verified where reproducible): **D5** write-path malfunctions now route through the rollback ladder (mid-write I/O error → `rolled-back`/`rollback-failed` with a new orthogonal `writeFailure` cause; unreadable file at pre-write re-verify → zero-write `stale-span` refusal) — the reviewer had empirically reproduced the partial-workspace throw; **D5-win** LSP URI→path now separator-normalized via the shared `normalizePath` (D3 comparison correct on Windows); **D5b** concurrent applies serialized per instance via a dedicated `applyMutex` (RED proved a real interleaving; `indexMutex` reuse would deadlock — cross-process concurrency remains a documented limitation); **D5c** genuinely-overlapping LSP edit sets now degrade to graph at plan derivation (`lspDegradation: overlapping-edits`, the spec's overlapping-range clause applied at plan time; `writeEdits` keeps its check as defense-in-depth). NIT (predictable temp-file name) recorded, deliberately unfixed. Reviewer's clean list independently re-verified position conversions, edit ordering, byte fidelity, D4 fast-path≡indexer semantics, watcher-race discrimination, chunked queries, jail reuse, and upstream-file fork discipline. Gates post-batch: targeted 204/204; **full suite 3117/0 (175 files)**; tsc + build clean. spec.md Edge Cases + data-model.md amended (writeFailure; lspDegradation two-value enum).

- **Integration Suite** (#90): full scrubbed suite at committed HEAD `eea5e1f`: **175 files / 3117 passed / 7 skipped / 0 failed**; tsc + build clean (D5-batch gates re-verified first-hand).
- **Reviewability Diff Gate** (#91, runner tasks-mode deferred → fallback evidence chain): actual reviewable size — Slice 1 src **+1,683** / tests +2,315; Slice 2 src **+1,419/−46** / tests +2,624/−72. Both materially exceed the 400-LOC advisory ceiling (scaffold estimate was 405 for the whole spec, WARN within-greenfield-allowance; growth = Clarify/Checklist-hardened taxonomy + apply ladder + D1–D5 remediation batches). Ceiling is advisory (PRSG-010 hard gate not installed); **disclosed prominently in both PR packets** with review-order + FR-traceability as the navigation mitigation; the ratified 2-slice split and the atomicity classifier's one-navigable-PR advisory dissent both stand as recorded.
- **Self-Review** (#92, orchestrator audit): (1) all gates G0–G7 PASS with evidence; 9 consensus resolutions, 4 human-ratified security items; 53/53 tasks; 5 defect batches (D1–D5) each TDD-remediated and re-validated same-session. (2) Single-source/unverified items, all recorded: Windows execution (gated + deferred, VM suspended — D5-win fix is precedent-correct but never run on win32), S2-H A/B cited-not-rerun at the gate, D5-win RED not reproducible on POSIX (disclosed), cross-process apply concurrency unserialized (documented limitation), `codegraph sync` lacks `--path` (UX seam observation). (3) Skeptic targets, all disclosed: reviewable-LOC ceiling overrun (packets lead with it), LSP path frequently degrading to graph on large TS repos pending the tsserver warm-up follow-up, doc-comment mentions deliberately leftover-only (FR-013). (4) Scope conformance: every changed line traces to a task or accepted finding; upstream-owned files additive-only (reviewer-verified); no session URLs anywhere; tracking-fork discipline held. **PASS with enumerated deferrals.**

- **PR review remediation round 1 (D6/D7, loop #96)**: PR #43 Copilot (2 threads) → **D7** truthful FR-003a probe-reason wording — `LspPathDisposition` distinguishes `unavailable-missing-command` from `unavailable-command-not-executable`; the local/parameter refusal never claims a server "did not respond" on the probe-only path; docstrings reconciled (slice-1 fix `85167f5` in an isolated worktree, threads replied + resolved, slice-1 suite 3022/0, PR #43 CI re-green) — forward-merged into the slice-2 branch (`e19be72`, clean ort merge, full suite **3124/0**). PR #44 CodeQL FAILURE + Copilot (7 threads) → **D6** (`c34c2e7`): all three `js/insecure-temporary-file` alerts fixed — crypto-random temp-sibling names, `mode: 0o600` on the flagged writes (including the restore-in-place sink the alert boilerplate obscured; confirmed against the CodeQL query source), `0o700` recovery dir; refuse-before-read enforced at LSP translation AND `countLspLeftovers` (stash-verified load-bearing; pre-fix the out-of-root read was swallowed as `server-crash` and silently degraded to a *successful* graph plan — worse than the reviewer's framing); `takeSnapshots` unreadable-file → zero-write `stale-span` refusal (Rung-3 now matches D5's Rung-3b pattern); rollback message keyed on actual cause (`writeFailure` / `danglingReferences` / sync-lock); temp-sibling cleanup on failed renames; `formatApplyResultTable` gained its first direct tests. 8+2 new tests across the round; CodeQL threads auto-closed on re-scan; all Copilot threads replied + resolved on both PRs.

---

## Post-Implementation Checklist

- [x] All tasks marked complete in tasks.md — 53/53 ticked at the T053 gate close
- [x] Build succeeds: `npm run build`
- [x] Tests pass — 3110/0 (175 files) at the 2026-07-12 D4 re-gate: `npm test` (vitest, full suite)
- [x] Linux suite green in Docker (`--init`)
- [x] Control-repo A/B: no retrieval regression (≥2 runs/arm, Sonnet floor) — 2026-07-11, express@4.21.2 vs baseline `4751425`: NO-REGRESSION (numbers in Implementation Progress row 3)
- [x] Self-repo dogfood UAT evidence recorded (rename in this repo, post-check green) — 2026-07-11/12: found + remediated D3 (LSP completeness degrade), post-fix re-run PASSED with tsc clean + 60/60 (Implementation Progress row 4)
- [x] CHANGELOG entry under `## [Unreleased]`
- [x] `server-instructions.ts` updated (and `.cursor/rules/codegraph.mdc` dogfood copy if applicable) — write-tool paragraph T047 (`00de5f7`); .mdc two-tool update (`cdc56d7`)
