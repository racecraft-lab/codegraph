...
- **Authoring or governing canonical project knowledge** — CodeGraph consumes committed OKF from any conforming producer and derives local indexes/renders; it does not own repository knowledge writes.
- **Requiring SpecKit Pro or another OKF producer** — repositories without an OKF bundle retain existing code-only behavior.
...
### 3.19 Auto-Updating Project Knowledge Wiki *(→ SPEC-019)*

- **AC-19.1**: `codegraph wiki` generates a markdown site — overview, OKF concept views, per-cluster chapters, per-flow walkthroughs, and hub-symbol pages — with deterministic structure from the unified code/knowledge graph and optional prose from the LLM layer (or agent bundle / heuristic skeleton).
- **AC-19.2**: Regeneration is incremental: only views whose underlying code nodes or OKF concept/section hashes changed re-render, and watch/sync events drive automatic updates in `--watch` mode.
- **AC-19.3**: Output is served in the web app at `/wiki` and written under `.codegraph/wiki/` as generated, ignored, fully reproducible render state; it is not canonical or a committable knowledge source.
- **AC-19.4**: Full wiki build on the fixture monorepo plus OKF bundle completes < 5 min with a local LLM endpoint; skeleton mode < 30 s.
- **AC-19.5**: Wiki generation never writes the discovered OKF bundle. Any code-derived prose that should become durable knowledge is exported as a provenance-bearing proposal for the owning producer workflow.
- **AC-19.6**: Rendered pages preserve concept type, freshness/status, citations, OKF links, and code-evidence links so generated prose cannot erase trust boundaries.
...
- **Knowledge ownership stays producer-neutral**: committed OKF is read-only input; CodeGraph owns only derived nodes, edges, FTS rows, vectors, and renders under `.codegraph/`.
...
| Auto-Updating Project Knowledge Wiki | AC-19.* | SPEC-019 | SPEC-011, SPEC-018, SPEC-027 | P1 |
...
5. An upstream merge performed after Phase 2 completes without structural conflicts in the new modules — proof the additive-first constraint held.
