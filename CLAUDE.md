...
- `src/db/` — `DatabaseConnection`, `QueryBuilder` (prepared statements), `schema.sql`, `sqlite-adapter.ts`. Backed by Node's built-in **`node:sqlite`** (`DatabaseSync`) — real SQLite with WAL + FTS5, exposed through a thin better-sqlite3-shaped adapter. The bundled runtime always ships Node ≥22.5, so `node:sqlite` is always available: **no native build step and no wasm fallback**. (Running from source needs Node ≥22.5 — the wider npm `engines` range `>=20 <25` and `MIN_NODE_MAJOR = 20` in `src/bin/node-version-check.ts` gate the thin-installer shim, which downloads the bundled ≥22.5 runtime; on Node 20–22.4 from source, `sqlite-adapter.ts` fails with its own "Node.js 22.5+" error.) `codegraph status` reports the live backend (`node-sqlite`, the sole backend).
- `src/extraction/` — `ExtractionOrchestrator`, tree-sitter wrappers, per-language extractors under `languages/` (one file per language — TS/JS, Python, Go, Rust, Java, Kotlin, Scala, C/C++, C#, VB.NET, PHP, Ruby, Swift, ObjC, Dart, Lua/Luau, R, Pascal, COBOL, CFML cfscript/cfquery…), plus standalone extractors for non-tree-sitter formats (`svelte-extractor.ts`, `vue-extractor.ts`, `astro-extractor.ts`, `liquid-extractor.ts`, `razor-extractor.ts`, `cfml-extractor.ts`, `mybatis-extractor.ts`, `dfm-extractor.ts` for Delphi). `parse-worker.ts`/`parse-pool.ts` run heavy parsing off the main thread.
- `src/resolution/` — `ReferenceResolver` orchestrates `import-resolver.ts` (with `path-aliases.ts` for tsconfig path aliases + cargo workspace member globs), `name-matcher.ts`, dynamic-dispatch synthesizers (`callback-synthesizer.ts`, `c-fnptr-synthesizer.ts`, `goframe-synthesizer.ts`, `swift-objc-bridge.ts`), and `frameworks/` — per-language/framework resolvers grouped roughly one file per ecosystem (Express, NestJS, React/React Native/Expo, Svelte(Kit), Vue/Nuxt, Astro, Laravel, Drupal, Rails, FastAPI/Django/Flask, Spring/Play, Gin/GoFrame, Axum/Cargo workspaces, ASP.NET, Vapor, Swift↔ObjC, CICS, Fabric). Frameworks emit `route` nodes and `references` edges.
...
- `src/mcp/` — MCP server (`tools.ts`, `engine.ts`, `transport.ts`) plus the **daemon architecture**: a per-project background daemon (`daemon.ts`, `daemon-manager.ts`, `daemon-registry.ts`, `proxy.ts`) serves queries over a unix socket / named pipe so multiple agent sessions share one warm index; `ppid-watchdog.ts`/`liveness-watchdog.ts` handle orphan reaping, `query-pool.ts`/`query-worker.ts` run queries off-thread. `server-instructions.ts` is what the server returns in the MCP `initialize` response — keep it in sync with the user-facing tool guidance.
- `src/installer/` — see below.
- `src/bin/codegraph.ts` — CLI (commander). Subcommands: `install`, `uninstall`, `init`, `uninit`, `index`, `sync`, `status`, `query`, `explore`, `node`, `files`, `callers`, `callees`, `impact`, `affected`, `daemon`, `unlock`, `telemetry`, `upgrade`, `version`; hidden: `serve --mcp` (what installed agent configs invoke) and `prompt-hook`. Anonymous usage telemetry is documented in `TELEMETRY.md` (design: `docs/design/telemetry.md`).
...
- Current targets: `claude.ts`, `cursor.ts`, `codex.ts`, `opencode.ts`, `gemini.ts`, `kiro.ts`, `antigravity.ts`, `hermes.ts` (shared helpers in `targets/shared.ts`).
...
- All installer changes need matching coverage in `__tests__/installer-targets.test.ts` — a parameterized contract suite (~90 tests, expanded per target) covering install idempotency, sibling preservation, uninstall reverses install, byte-equal re-runs returning `unchanged`, and partial-state recovery for Codex.
...
<!-- SPECKIT START -->
SPEC-025 (plugin platform mechanics spike) is complete on worktree
`025-plugin-platform-spike` and under review — the decision document is
`docs/design/plugin-channel-decision.md` (spec artifacts under
`specs/025-plugin-platform-spike/`); PR pending.
...
hook, skills, and agents) is downstream of SPEC-025's decision document and
becomes ready to scaffold once the SPEC-025 PR merges.
<!-- SPECKIT END -->
