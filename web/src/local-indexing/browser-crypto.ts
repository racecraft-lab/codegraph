  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let index = 16; index < 64; index += 1) {
      ...
      const right =
        ...
        (schedule[index - 2]! >>> 10)
      schedule[index] =
        (schedule[index - 16]! + left + schedule[index - 7]! + right) >>> 0
    }
...
export function createHash(algorithm: string) {
  if (algorithm !== "sha256") {
    throw new TypeError(`Unsupported browser hash algorithm: ${algorithm}`)
  }
  let value = ""
  return {
    update(next: string) {
    ...
    digest(encoding: string) {
      if (encoding !== "hex") {
        throw new TypeError(`Unsupported browser digest encoding: ${encoding}`)
      }
      return sha256(value)
    },
  }
