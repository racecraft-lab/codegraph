...
- **FR-050**: Browser-local workspace controls MUST be keyboard reachable with visible focus order for local/server switching, Open local folder, snapshot import fallback, progress/cancel, reconnect, refresh, semantic opt-in, server-only disabled surfaces, and delete confirmation; focus MUST return to the initiating control or move to the relevant status/alert after picker dismissal, cancellation, failure, completion, reconnect denial, busy state, quota block, and delete completion. *(UX Accessibility; Q1, Q2, Q4, Q6, Q13, Q20)*
- **FR-051**: Browser-local long-operation UI MUST expose progress and terminal states through assistive-technology-readable status or alert semantics, preserve usable mobile layouts without horizontal scrolling for controls/status content, honor reduced-motion preferences by avoiding non-essential progress or status animations, and provide a non-drag snapshot-import path wherever snapshot import is supported. *(UX Accessibility; Q4, Q13, Q20, Q21)*
- **FR-052**: If delete is requested while browser-local indexing, refresh, embedding, cancellation, or worker cleanup is active, the app MUST show the affected operation and repository, require explicit confirmation of whether deletion cancels the active operation, disable local reads/actions while deletion is in progress, and finish in either a deleted state or the previous readable state with a stable failure reason. *(UX Lifecycle; Q13, Q18, Q20)*
- **FR-053**: Browser source traversal for picked folders and dropped snapshots MUST derive accepted paths only from handle ancestry or drop-entry metadata, normalize separators to repository-relative POSIX paths, and reject absolute paths, empty segments, `.`, `..`, separator-normalized escapes, duplicate normalized paths, unsupported entry kinds, recursive cycles, and configured file/depth/count/byte traversal budget overruns before reading file text; rejected entries MUST become bounded warnings and MUST NOT enter the source cache, graph, semantic queue, URLs, or logs. *(Security; AC-7.1, AC-7.4; Q4, Q7, Q11)*
- **FR-054**: Browser-local source and symbol-source views MUST render repository text as inert text tokens through the shared source-pane model, never by assigning repository content to raw HTML sinks, script-capable documents, iframes, blob/object URLs, CSS URLs, or syntax-highlighter HTML; HTML/script-like bytes, `javascript:` text, and URL-looking strings from source files MUST display literally, MUST NOT auto-load linked resources, and MUST be covered by tests that fail if source text executes in the application origin. *(Security; AC-7.2, AC-7.3; Q14, Q16)*
- **FR-055**: Browser-local worker shutdown, tab close, clean close requests, worker crash, and failed delete cleanup MUST close SQLite statements and database handles before graph-file deletion or VFS pause/release, release the per-repository Web Lock when ownership ends, and treat any stale ownership or in-progress metadata found on next open as recoverable status that cannot open the database concurrently or mark the repository complete. *(Data Integrity; AC-7.2, AC-7.4; Q9, Q13, Q18)*
- **FR-056**: Browser-local semantic vectors MUST converge only when graph generation, semantic generation, model id, dimensions, stored vector byte length, and input hashes match the current Embedding Profile; graph generation changes, model or dimension changes, partial embedding responses, cancellation, or endpoint failure MUST leave keyword search usable, mark semantic coverage resumable or stale with stable status, and avoid serving mixed-dimension or stale-model vector results. *(Data Integrity, Semantic; AC-7.3; Q14, Q15, Q16)*
- **FR-057**: Manual refresh success MUST report deterministic added, changed, deleted, unchanged, skipped-warning, and source-cache write/remove counts, and the published accepted-file manifest, graph rows, source-cache entries, repository status, and semantic stale/resume metadata MUST all refer to the same generation. *(Data Integrity; AC-7.2; Q12, Q13, Q15)*
- **FR-058**: Recovery coverage MUST explicitly include crashes or failures after source-cache staging before graph write, after graph write before registry publish, after registry publish before visible status update, during migration, during quota exhaustion, and during delete cleanup; each covered case MUST preserve or restore the prior readable generation, expose a non-complete status, and clean incomplete staging on the next open without deleting source-folder files. *(Data Integrity; AC-7.2, AC-7.4; Q9, Q13, Q18, Q20)*
- **FR-059**: Browser-local index and refresh work MUST declare and enforce bounded read/message/progress budgets for file batches, snapshot transfer payloads, worker RPC payload size, and progress event cadence; budget overruns MUST become stable bounded warnings or recoverable failures before unbounded structured-clone, source-cache, graph, semantic, URL, or log work begins. *(Performance; AC-7.1; Q8, Q17, Q19)*
- **FR-060**: Local keyword search, graph navigation, and bounded impact implementations MUST publish query-plan evidence for the deterministic self-repository suite, including SQLite `EXPLAIN QUERY PLAN` or equivalent browser-SQLite plan output, index/FTS usage, row-limit/candidate caps, and any intentional bounded scan rationale. *(Performance; AC-7.2; Q19)*
- **FR-061**: Browser-local runs MUST release or bound parser trees, grammar/parser caches, SQLite statements and database handles, SAH-pool/VFS ownership, worker instances, transferable buffers, and vector matrices after completion, cancellation, delete, worker crash, and repeated index/embed cycles; evidence MUST record memory or resource high-water and post-cleanup state for repeated self-repo runs. *(Performance, Data Integrity; AC-7.1, AC-7.2; Q8, Q9, Q13, Q19)*
- **FR-062**: Package and static-host verification MUST record the local-indexing asset inventory and byte sizes, initial route bundle impact, and request sequence proving SQLite, tree-sitter core, and grammar WASM assets are not eagerly requested before local indexing and accepted-language demand; any missing, empty, uncopied, CDN, or dev-server-only asset remains fail-closed per FR-046. *(Performance, Shipping; AC-7.1; Q3, Q7, Q19)*
- **FR-063**: Semantic embedding work MUST run as a separate cancellable/resumable worker operation after keyword publication, batch endpoint and vector writes under documented budgets, keep progress/cancel controls actionable, and preserve the 150 ms p95 local keyword/search/graph/impact read target while embedding is active, paused, failed, or cancelled. *(Performance, Semantic; AC-7.3; Q14, Q15, Q19)*

### Acceptance Criteria Coverage

- **AC-7.1 - Deliberate local import**: FR-001 through FR-010, FR-016, FR-025, FR-027, FR-031, FR-037 through FR-041, FR-047 through FR-048, FR-053, and FR-059 through FR-062 cover explicit folder selection, local indexing, persistence, worker progress, asset delivery, deterministic source selection, traversal-safe source admission, bounded read/message/progress budgets, transactional publication, cross-runtime parity, resource cleanup, and lazy packaged asset behavior.
- **AC-7.2 - Core graph experience**: FR-010 through FR-017, FR-026, FR-029, FR-030, FR-050, FR-054 through FR-058, FR-060, and FR-061 cover overview, search, source, relationships, graph, impact, reconnect, refresh, storage, deletion, query-plan evidence, worker close/release, generation consistency, recovery, keyboard reachability, focus management, and inert source rendering.
- **AC-7.3 - Local-first privacy and semantic opt-in**: FR-021 through FR-024, FR-029, FR-054, FR-056, and FR-063 cover no default egress, explicit semantic consent, memory-only credentials, secure transport, resumability, cancellable/non-blocking embedding work, keyword fallback, vector convergence, and malicious source text that remains inert in the application origin.
- **AC-7.4 - Graceful degradation and busy states**: FR-004, FR-018 through FR-020, FR-024, FR-027, FR-030, FR-033 through FR-034, FR-043, FR-045 through FR-046, FR-049, FR-051 through FR-053, FR-055, and FR-058 cover snapshot fallback, unsupported browsers, busy tabs, quota and permission messages, static-host behavior, accessible fallback controls, active-operation deletion, traversal-safe dropped snapshots, stale ownership recovery, crash recovery, and the three-browser capability matrix.
...
- **SC-012**: Browser-local import, reconnect, refresh, cancel, busy, quota, unsupported, server-only-disabled, semantic opt-in, and delete flows are operable by keyboard, maintain visible focus, expose progress and terminal state changes to assistive technology without stealing focus, avoid horizontal scrolling at 320 CSS px width, and suppress non-essential motion when reduced-motion preference is active in all covered UX tests.
- **SC-013**: A malicious source fixture containing traversal-shaped paths, duplicate normalized paths, recursive directories, unsupported entry kinds, oversize files, binary files, and unreadable files produces only accepted-safe source entries plus bounded warnings; no rejected entry appears in the source cache, graph, semantic input, route URLs, or logs.
- **SC-014**: A malicious source fixture containing `<script>`, event-handler attributes, SVG/script-like markup, CSS URLs, `javascript:` text, and URL-looking strings is displayed as literal source text; no code executes, no linked resource auto-loads, and no source-derived network request occurs in the application origin.
- **SC-015**: Crash, worker-failure, quota-exhaustion, migration-failure, and database-write failure tests across source-cache staging, graph write, registry publish, status update, and delete cleanup preserve or restore the prior readable generation, never expose a falsely complete status, and remove incomplete staging on the next open in all covered cases.
- **SC-016**: Incremental refresh tests with one added file, one changed file, one deleted file, one unchanged file, and one skipped-warning file publish matching graph rows, source-cache rows, accepted-manifest entries, semantic stale/resume metadata, and visible refresh counts for the same generation.
- **SC-017**: Semantic resume tests show that model id, dimension, graph-generation, input-hash, cancellation, and partial-response changes either resume the same convergent vector generation or mark semantic results stale/unavailable while keyword search remains usable.
- **SC-018**: Web Lock and worker-close tests show that a clean close, tab close, worker crash, and delete cleanup release ownership or report recoverable stale ownership without opening the same local repository database concurrently.
- **SC-019**: The self-repository Performance evidence includes enforced file batch, worker payload, snapshot transfer, progress cadence, embedding batch, and vector-write ceilings; tests prove budget overruns are reported before unbounded structured-clone, read, graph, semantic, URL, or log work begins.
- **SC-020**: Packaged CLI-served and HTTPS static-host evidence records local-indexing asset byte sizes, initial route bundle impact, and request order showing no SQLite, tree-sitter core, or grammar WASM request before local indexing and accepted-language demand.
- **SC-021**: During an active semantic embedding run on the completed self-repository keyword index, local keyword search, graph navigation, and bounded impact reads still meet the 150 ms p95 target, cancellation prevents further endpoint calls before the next documented batch, and resumable semantic status remains consistent with the current graph generation.
