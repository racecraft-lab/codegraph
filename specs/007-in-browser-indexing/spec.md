...
- **AC-7.1 - Deliberate local import**: FR-001 through FR-010, FR-016, FR-025, and FR-027 cover explicit folder selection, local indexing, persistence, worker progress, asset delivery, and deterministic source selection.
...
- **AC-7.4 - Graceful degradation and busy states**: FR-004, FR-018 through FR-020, FR-024, FR-027, and FR-030 cover snapshot fallback, unsupported browsers, busy tabs, quota and permission messages, and static-host behavior.
...
- **Local Repository**: A browser-owned CodeGraph repository record with identity, display name, runtime type, status, storage ownership, and last successful index metadata.
...
- **Worker Operation**: A browser-local index, refresh, query, embed, cancel, or close operation with progress, result, and error status.
...
- **Embedding Profile**: Non-secret endpoint and model settings plus resumable semantic state; bearer credentials are never part of this entity.
- **User-Visible Error**: A recoverable or terminal condition with category, affected operation, safe details, and the next user action.
...
- **SC-008**: On documented current desktop Chromium conditions, this CodeGraph repository completes browser keyword indexing in at most 60 seconds without embeddings while the UI remains responsive.
- **SC-009**: On the same self-repository benchmark, local keyword search, graph navigation, and bounded impact reads meet p95 latency at or below 150 ms.
