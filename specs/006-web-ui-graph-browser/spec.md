...
- Non-loopback, token-bound API requests return unauthorized when the presented CodeGraph server bearer token is missing or invalid; non-loopback startup without a configured server token is treated as backend unreachable or unavailable because the server refuses to bind.
