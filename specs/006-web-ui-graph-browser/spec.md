...
- Q: What auth/token/secret boundary should the browser follow for API and provider credentials? A: Browser calls stay same-origin; loopback defaults need no credentials, non-loopback may use only the CodeGraph server bearer token, and provider URL/model/key material remains backend-only through `CODEGRAPH_LLM_*`.
...
- Q: What local/Docker serving expectation should SPEC-006 require? A: Use local/package serving as the supported path: `codegraph serve --web` must serve packaged static assets locally, package validation must prove web assets are included, offline/no-CDN behavior must be tested, and container or non-loopback use must be documented through the existing `--host`, `--port`, and `CODEGRAPH_SERVER_TOKEN` rules. SPEC-006 does not require a full production Dockerfile or hosted demo path.
...
- **FR-040**: Repo-scoped read workflows MUST use the existing repo-selection contract: `/api/status` and `/api/repos` are not repo-scoped, while search, node, callers, callees, graph, impact, flows, and clusters use `?repo=<repo-id>` and re-analysis uses `/api/reindex/{repo}`.
- **FR-041**: Any SPEC-006 chat endpoint MUST be same-origin, backend-routed, and implemented as a thin adapter over SPEC-018 rather than a separate LLM provider implementation.
...
- **FR-045**: Re-analysis progress MUST use the existing SPEC-005 job and SSE contracts, including one active job per repo, duplicate-job `409`, snapshot-on-connect, progress events, terminal `done` or `error`, heartbeat comments, and no Last-Event-ID replay guarantee.
...
- **FR-052**: SPEC-006 MUST validate local package serving through `codegraph serve --web` and document a container recipe or docs path using package-shipped assets, existing host and port behavior, and existing non-loopback bearer-token requirements.
...
- **SC-011**: Maintainers can build package assets, run `codegraph serve --web`, verify the app loads packaged web assets with external network access blocked except the local backend, and follow documented container or non-loopback guidance using existing host, port, and `CODEGRAPH_SERVER_TOKEN` rules.
