...
- A bounded variable relationship produces many candidate paths before a top-level `LIMIT` or row cap.
...
- Stored opaque JSON for return-only public fields is malformed or has the wrong top-level public shape.
- A result exceeds the default or hard row cap.
- A bounded result's canonical JSON payload would exceed the fixed machine-output payload ceiling.
- A query reaches the fixed five-second deadline.
- CLI stdin contains malformed or oversized UTF-8 input.
- A string literal contains raw control characters, a raw line terminator, an invalid escape sequence, or a Unicode escape form.
...
- **FR-013**: JSON-valued or array-valued public fields such as `metadata`, `decorators`, and `typeParameters` MAY be returned whole as opaque values but MUST NOT be used by `WHERE` in v1. Nested access, indexing, comparison, string predicates, and null predicates over those fields MUST be rejected as unsupported subset syntax. Stored opaque JSON conversion MUST parse only the expected public shape: relationship `metadata` as an object, node `decorators` as an array, and node `typeParameters` as an array. Malformed JSON, scalar JSON, or the wrong top-level shape for those fields MUST surface as null/absent in the public value, MUST NOT expose the raw storage string, and MUST NOT be silently coerced to an object, array, string, boolean, or number.
...
- **FR-016**: Query literals MUST be accepted in query text and bound safely by the engine; caller-supplied external parameter objects MUST NOT be added in this version. V1 string literals MUST use single quotes only. Literal single quotes and backslashes MUST be represented with `\'` and `\\`; the only supported string escapes are `\'`, `\\`, `\n`, `\r`, `\t`, `\b`, and `\f`. Raw line terminators, NUL, other raw control characters, Unicode escape forms such as `\uXXXX`, invalid escapes, incomplete escapes, double-quoted strings, and literal concatenation MUST be rejected with a syntax or unsupported-subset diagnostic. Decoded string values MUST compare exactly without Unicode normalization and MUST always be emitted as bound SQLite parameters, never concatenated into SQL text.
...
- **FR-020**: Results MUST default to at most 100 rows and clamp any explicit limit to a hard cap of 1,000 rows. Execution MUST inspect at most `effectiveCap + 1` rows, or use an equivalent bounded existence check, return only `effectiveCap` rows, set `truncated: true` only when an additional row exists, include `effectiveCap`, and MUST NOT compute or expose an unbounded `totalRows`. Variable-path execution MUST carry depth, direction, and relationship-simple visited-edge state inside recursive expansion and MUST apply bounded frontier or output guards before any final top-level `LIMIT` so row caps cannot hide unbounded recursive materialization. Canonical machine output for the shared result contract MUST enforce a fixed 1 MiB UTF-8 serialized JSON payload ceiling after deterministic serialization and before CLI/MCP emission; if the ceiling would be exceeded, all surfaces MUST return stable diagnostic `CYPHER_OUTPUT_TOO_LARGE` with no partial rows and guidance to narrow `RETURN`, `MATCH`, or `LIMIT`.
...
- **SC-010**: Representative performance probes over realistic graph density record query-plan evidence for variable paths, stable ordering, count/grouping, row-cap truncation, canonical output-size rejection, and edge-index use; each probe stays within the fixed row, path, payload, and timeout bounds or returns the documented diagnostic/timeout state.
