...
| `lspDegradation` | `incomplete-coverage` \| `overlapping-edits` \| `unsupported-edits`? | Optional; present only when an `ok` LSP result was unusable and the rename degraded to the graph path: coverage missing a span-verified graph file (D3), genuinely-overlapping edit ranges detected at plan derivation (D5c, spec's overlapping-range clause), or an edit shape the writer cannot honor — a documentChanges resource operation, or an in-root edit with a multiline range / empty live-derived oldText (A2). |
...
The tier is the resolver edge's `resolvedBy` category (stored in `edges.metadata` JSON, **not** a column). Where one label historically conflated a validated structural resolution with a naming guess (`instance-method`), the resolver now emits a **distinct label for the validated branch** (`instance-method-decl`, from `resolveMethodOnType`), so the tier is an exact function of the label — **never a runtime-configurable threshold**. The legacy `instance-method` split (confidence ≥ 0.85 → `exact`) is retained ONLY as a backward-compatibility fallback for edges in already-built indexes that predate the new label. `resolvedBy` union: `src/resolution/types.ts:42`. `provenance` union (`src/types.ts:62-67`): `tree-sitter` | `scip` | `heuristic` | `lsp` (base resolved edges set none → persist NULL).
...
| **exact** | `instance-method-decl` — **declaration-recovered** (all conf) | `resolvedBy:'instance-method-decl'`, emitted whenever the receiver type came from an actual declaration and `resolveMethodOnType` confirmed `Type::method` exists. **Legacy fallback:** an `instance-method` edge with conf ≥ 0.85 (pre-relabel indexes). | `resolveMethodOnType()` relabels once (`name-matcher.ts:499`); reached from sites `:840,868,909,940,962,974,978,1491,1531,1559` (conf 0.8–0.85) | Structurally validated against a real type — the label alone is the discriminator, no threshold. |
| **heuristic** | `instance-method` — **capitalization-guess / word-overlap branch** | `resolvedBy:'instance-method'` emitted DIRECTLY (never via `resolveMethodOnType`), so on a fresh index this label is only ever a guess. **Legacy fallback:** a pre-relabel declaration-recovered `instance-method` at conf 0.8 under-classifies here — the SAFE mis-direction (exact→heuristic gates behind confirmation, never the reverse). | `matchMethodCall` Strategy 2 (`name-matcher.ts:1608-1636`, conf 0.8) and Strategy 3 word-overlap (`:1641-1703`, conf 0.7/0.65) | Guesses a class by capitalizing the receiver / camel-case overlap. **This is the explicit split** — the validated branch carries its own label above. |
...
| Name occurrences TO the target (graph path) | `getReferencesToNode(targetId)` → every incoming NAME-OCCURRENCE edge, **not just `references`**: `RENAME_RELEVANT_EDGE_KINDS` = `references` / `calls` / `imports` / `extends` / `implements` / `type_of` / `returns` / `instantiates` / `overrides` / `decorates` (every kind whose SOURCE position textually names the target; the structural `contains` / `exports` are excluded — `contains` has no position, and re-export & import specifiers are recorded as `imports`). Yields `edges.source/line/col/metadata/provenance`. **Span verification (FR-005) is the per-edge filter that makes broad inclusion safe** — it drops any occurrence whose live slice ≠ the old name, so an `instantiates` / `decorates` position pointing at the `new` / `@` sigil is dropped, never mis-edited. (SPEC-010 D1: the `references`-only scope missed call sites + import specifiers, leaving broken code on apply that the FR-018 touched-file post-check could not see.) | `queries.ts` (`getReferencesToNode`, `RENAME_RELEVANT_EDGE_KINDS`) |
...
New statements to add (additive): the name-occurrence-to-node scoped select (`getReferencesToNode`, widened past `references` to `RENAME_RELEVANT_EDGE_KINDS` for D1) reused across plan + post-check, and touched-file-scoped variants of `getUnresolvedByName` / `getNodesByName` for the FR-018 dual assertion. No inline SQL (workflow constraint).
