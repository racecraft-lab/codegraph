...
- **Serialization**: `sync()` runs under `this.indexMutex.withLock(...)` (`index.ts:1268`) plus a cross-process `fileLock` (`index.ts:1270`); the file watcher's callback calls the same `sync()` (`index.ts:1495`) on the same shared `CodeGraph` instance for an MCP-served apply, so apply's re-sync is already totally ordered with watcher-triggered syncs on one mutex — no additional mutex-holding or watcher suspension is needed. A lock-contended/no-op sync returns a zero-shape `SyncResult` (`filesChecked: 0`, `durationMs: 0` — `index.ts:1507`) — FR-018 requires treating exactly that zero-shape as an **apply failure → rollback**, not any result with `filesModified = 0`: the apply engine checks specifically for the lock-failure zero-shape (mirroring the same discrimination `watch()`'s callback already applies, `index.ts:1495-1509`) and proceeds to the post-check on any other result, including a real empty re-sync (`filesChecked > 0`, `filesModified = 0`) left by a watcher-driven `sync()` that raced ahead and already re-resolved the touched files. **The post-check never runs against an un-updated graph.**
