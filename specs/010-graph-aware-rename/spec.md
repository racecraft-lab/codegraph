...
- **FR-004**: Each edit MUST be assigned a confidence of `exact` or `heuristic`, where `exact` denotes a language-server workspace edit or a span-verified graph reference and `heuristic` denotes a lower-assurance derivation (name-match or synthesized provenance). [NEEDS CLARIFICATION: precise mapping of edge-provenance classes to tiers — which provenance values (e.g. AST-direct, import-resolved, LSP-verified graph edge, name-matched, heuristic-synthesized) count as `exact` vs `heuristic`, and whether heuristic-synthesized edges are ever emitted as applicable edits or only counted as leftover-mention FYIs. The design concept fixes the gate semantics (Q3) but explicitly defers this tier-assignment table to Clarify/plan.]
...
- **FR-013**: The plan MAY report a count of leftover textual mentions of the old name as an informational FYI, but MUST NOT edit them (design concept Q9).
...
- **FR-017**: Writes MUST be confined to the workspace root (a path jail) and MUST respect `.gitignore`; no clean-git-worktree precondition is imposed (design concept Q4).
- **FR-018**: Apply MUST take a pre-write snapshot of every touched file, then perform a targeted re-sync of the touched files, then run a post-check asserting zero dangling references to the old name, where a dangling reference means a graph reference to the renamed symbol that still carries the old name after re-sync (design concept Q5).
...
- **FR-020**: A rename MUST be atomic through verification — on any gate, span, or post-check failure the workspace MUST be left byte-identical to its pre-apply state.

**MCP surface (Slice 2)**

- **FR-021**: An MCP tool `codegraph_rename` MUST expose the identical plan/apply contract as the CLI: dry-run by default, with side effects only on an explicit `apply: true` parameter (design concept Q7).
- **FR-022**: The `codegraph_rename` tool MUST always be exposed on the MCP surface; it is never hidden behind an opt-in gate (design concept Q7).
- **FR-023**: Every expected or recoverable condition — ambiguity, a heuristic-gated apply, a stale span, a project that is not indexed, an unsupported kind — MUST return a success-shaped response carrying actionable guidance, never an error-shaped (`isError`) response (design concept Q7; constitution Principle VI).
- **FR-024**: Adding the tool MUST NOT regress retrieval on a control repository, validated with the A/B methodology (≥2 runs per arm, on the floor model) before merge (constitution Principle VI).
- **FR-025**: The agent-facing tool guidance (the single source of truth returned in the MCP initialize response) MUST be updated to describe the new write tool and its dry-run-by-default / explicit-apply contract.
...
- **Rename Edit**: a single change within one file — its file path, its span/range, the old text and the new text, its confidence tier, and the provenance that produced it.
...
- **Confidence Tier**: the two-valued rating on each edit — `exact` or `heuristic` — that drives the apply gate.
...
- **SC-006**: Every recoverable condition (ambiguous, heuristic-gated, stale span, not indexed, unsupported kind) is delivered as actionable, success-shaped guidance, so none of them causes an agent to stop using the tool.
...
- The answers recorded in `docs/ai/specs/.process/SPEC-010-design-concept.md` (Q1–Q11) are authoritative and are reaffirmed by this spec; the single item that concept explicitly defers to Clarify — the exact/heuristic tier-assignment table — is carried as the one open-clarification marker in FR-004.
