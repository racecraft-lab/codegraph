...
**Reviewability**: The spec's ratified **2-slice split** is binding. Budget: ~200 reviewable LOC/slice, ~5 production files, single primary surface (`src/refactor/`). If task generation would push a slice past 400 reviewable LOC / 6 production files / 15 total files / >1 primary surface, stop and re-split rather than adding tasks (tasks-template Reviewability).
...
- **Ordering rule**: EVERY Slice-1 task (T001–T027) completes before ANY Slice-2 task (T028+). Slice 1 files never import Slice 2 files; Slice 2 imports Slice 1 (notably `span-verify.ts`, the shared seam).
...
- [ ] T008 Reviewability checkpoint — confirm the planned task/file scope stays within the ratified 2-slice budget (single primary surface `src/refactor/`; ~200 reviewable LOC and ≤5 production files per slice) and record the split decision. Stop and re-split if exceeded, rather than adding implementation tasks. (No `src/` change.)
