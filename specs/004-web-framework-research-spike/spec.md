...
As a maintainer, I can inspect screenshot evidence and prototype notes showing the selected stack rendering representative CodeGraph data, including a chosen-stack graph-rendering bake-off and the roadmap's 1k-node/60fps target, so the highest-risk visualization path is proven before production work begins.
...
**Independent Test**: Can be tested by following the reproduction notes, reviewing the committed screenshots, and confirming the prototype uses representative data from this repository plus a 1k-node/60fps target.
...
2. **Given** the 1k-node/60fps target, **When** prototype results are reviewed, **Then** the notes describe whether the target was reached, what frame-rate or interaction-smoothness signal was measured, and any limitations that later specs must handle.
...
- What happens when the chosen stack renders representative data but struggles with the 1k-node/60fps target?
...
- **FR-010**: The spike MUST run a chosen-stack graph-rendering bake-off before final prototype evidence is recorded, covering canvas/WebGL force-graph options where available, and the prototype MUST include or simulate the roadmap's 1k-node/60fps graph-rendering target and record whether that target was reached.
...
- **FR-014**: The spike MUST include a self-repo UAT step that exercises representative CodeGraph data from this repository and records the commands, outcome, and limitations in both `docs/design/web-framework-decision.md` and `specs/004-web-framework-research-spike/.process/uat-runbook.md`.
- **FR-015**: The spike MUST keep committed artifacts docs/process-focused and MUST NOT add production server code, production web UI code, in-browser indexing code, LSP facade or WebSocket endpoint code, maintained prototype source, or any other production behavior.
...
- **Split decision**: This remains one spec because it produces a bounded decision document, small screenshot evidence assets, and recorded prototype notes with no production web code. Later production implementation belongs to SPEC-005, SPEC-006, and SPEC-007.
...
- **Prototype Evidence**: Screenshot assets and notes proving the selected stack can render representative CodeGraph graph data, including the 1k-node/60fps target.
...
- **SC-004**: The prototype evidence includes at least two committed browser screenshots: one for representative self-repo graph data and one for the 1k-node/60fps target or the closest achieved fallback. Each screenshot is captured at 1280x720 or larger unless a fallback is documented, and the decision document captions make the screenshot evidence readable by naming the visible graph region, selected or focused state if any, dataset, node/edge counts, capture method, and frame-rate or interaction-smoothness signal. If no screenshot can be captured after the required fallback ladder, the decision document MUST mark the screenshot requirement as unmet or pass-with-limitation, preserve reviewer-readable substitute evidence, and record the downstream impact; absence of PNGs MUST NOT be silently accepted as success.
- **SC-005**: The reproduction notes and UAT runbook preserve the recorded prototype validation result, data shape, commands, limitations, and committed screenshot evidence. A maintainer can recreate an equivalent scratch rerun from the documented local steps and package-shipped or locally generated assets, but SPEC-004 does not promise a turnkey clean-checkout rerun of uncommitted temporary scripts, prototype source, or generated data. The decision document also states the SPEC-005-ready serving model for embedded static assets and local graph APIs.
...
- If the full 1k-node/60fps target cannot be reached, the spike records the closest achieved result, the blocker, and the downstream implication instead of silently lowering the bar.
- SPEC-004 records the static-asset, container, and SPEC-005 handoff requirements only; SPEC-005 implements the local HTTP server/static mount, SPEC-006 implements the production web app and `copy-assets` wiring, SPEC-007 implements browser-side indexing, and later LSP/WebSocket work remains outside this spike.
