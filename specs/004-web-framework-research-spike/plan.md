...
**Target Platform**: Local-first, package-shipped web assets served by CodeGraph in later specs, plus a standalone container recipe for deploy-anywhere users.
...
**Constraints**: No production code. No non-permissive or source-available-only dependencies. No CDN or runtime hosted services. No changes to extraction, retrieval, MCP, SQLite schema, installer behavior, or release flow.
...
| VII. Local-First, Private, Zero Native Dependencies | PASS | Web capabilities remain dormant and opt-in; candidates and graph libraries must run offline with permissive pure JS/WASM dependencies. |
...
| Gate | Pass threshold |
|------|----------------|
| Self-host anywhere | Official docs show a local static or Node/container serving path that does not require a proprietary platform. |
| Offline/package-shipped assets | Production UI can run with npm package-shipped JS/CSS/assets and locally generated CodeGraph data; no required CDN fetches at runtime. |
| No hosted-service runtime dependency | Startup, routing, graph rendering, and data loading work without hosted SaaS, cloud functions, hosted databases, or remote auth. |
...
| Package footprint | Minimal candidate build plus selected graph renderer is plausibly package-shippable; implementation must measure compressed and uncompressed production asset size and record any candidate whose required shipped footprint exceeds the SPEC-004 threshold chosen in the decision doc. |
...
|-----------|--------|----------------------|
| UX and graph-interaction fit | 35 | Quality of interactive graph rendering path, routing/state ergonomics, browser performance, and future graph-browser UX headroom. |
...
2. Gather live metadata for each candidate and any graph-rendering library under consideration: current npm version, license, dependency posture, unpacked/package footprint, repository URL, latest release or commit activity, archive/deprecation status, and relevant issue health.
3. Record every source with access date and link in `docs/design/web-framework-decision.md`.
...
- Browser screenshot capture using the available browser automation path, with local Playwright or equivalent fallback recorded if needed.
- Performance and reproduction notes: commands, browser, machine context, node/edge counts, first visible render timing, interaction notes, asset size, and limitations.
...
The decision document must include: executive recommendation, design-concept quotes, candidate matrix, hard-gate results, weighted scoring, selected stack, selected graph-rendering library, shipping strategy, prototype method, screenshot references, self-repo UAT result, reproduction steps, known risks, and deferred work mapped to SPEC-005/SPEC-006/SPEC-007.
...
7. Record outcome, commands, counts, timing, browser/tooling path, and limitations in `docs/design/web-framework-decision.md`.
