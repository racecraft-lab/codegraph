...
- `.ml` and `.mli` files both report as the public `ocaml` language. `.ml` files use `tree-sitter-ocaml.wasm`; `.mli` files use `tree-sitter-ocaml_interface.wasm` through an internal grammar-selection path, not a second public language. Pairing is same normalized directory plus basename only when unique; ambiguous pairings fail closed.
