...
- **FR-026**: The server MUST start, serve, and stop cleanly under standard process termination, releasing the bound port on exit so a subsequent start can re-bind. (When a job is in flight at shutdown, the job is aborted via its AbortSignal within a bounded grace period — see FR-023.)
