...
| **III. Surgical Changes / Fork Discipline** | PASS | All new capability in the new `src/server/` module. Upstream-owned diffs are minimal and enumerated: `src/bin/codegraph.ts` (+1 `--web` option, +1 action branch, +1 mutual-exclusion guard); `src/embeddings/config.ts` (delete private `isLoopbackHost`, import shared); `src/utils.ts` (+ shared `isLoopbackHost`); `src/mcp/engine.ts` + daemon request handler (+1 additive `rearmWatcher` control op); `src/mcp/session.ts` + engine/query-pool wiring (+1 additive structured READ-ONLY JSON-RPC method `codegraph/read` returning library `Node`/`Edge`/`Subgraph`/stats — **plan amendment, human-ratified 2026-07-11**: the socket's MCP `tools/call` surface returns id-less markdown, so the REST endpoints' structured wire shapes require this read channel; FR-002/Q1 upheld verbatim — the daemon executes all reads over its socket against the one warm index; FR-021 forbids only *indexing* RPCs); `package.json` copy-assets (+ copy `openapi.yaml`). No upstream file is rewritten. |
...
| **VI. Retrieval Performance Is a Regression Surface** | PASS (guarded) | The read API forwards existing daemon queries verbatim — no change to MCP tool output, `getExploreBudget`/`getExploreOutputBudget`, error shaping, or edge synthesis. The two `src/mcp/` touches (additive `rearmWatcher` control op; additive `codegraph/read` structured read method, human-ratified amendment) are additive dispatch cases that never alter an existing tool response, budget, or error shape. Guardrail: run the retrieval-guardian check before EACH slice PR that touches `src/mcp/` (slice 1: codegraph/read; slice 2: rearmWatcher). |
