...
- [x] CHK006 Is the 400 `invalid_request` response (malformed/negative params) documented on the param-taking read endpoints (`callers`, `callees`, `graph`) and not only on `/api/search`, given FR-015a mandates 400 for malformed params? [Resolved → FR-025 + openapi 400 added to /api/callers, /api/callees, /api/graph (impact takes no client params after CHK012, so no 400)]
