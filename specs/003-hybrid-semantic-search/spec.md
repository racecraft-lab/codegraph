...
- **Index staleness mid-session**: A cheap per-query staleness probe (matching-model vector count from `getEmbeddingCoverage` + the `embedding_model`/`embedding_dims` scalars and the monotonic `vectors_write_version` counter from `project_metadata` — FR-008b; there is no `data_version` column) invalidates and rebuilds the in-memory matrix cache when the index changed.
...
- **FR-009**: The KNN scan MUST use a lazily built in-memory single-precision matrix cache covering all vectors whose model matches the active provider's model, invalidated per query by a cheap staleness probe (matching-model vector count + the `embedding_model`/`embedding_dims` scalars + the `vectors_write_version` counter — FR-008b; there is no `data_version` column); resident memory equals count×dims×4 bytes and MUST be documented (Q6).
...
strings are pinned literals (asserted verbatim by the FR-015 / US3 degradation
tests' footer assertions, run under `npm test`), this invariant is mechanically
enforced, and any wording change is a review gate that MUST preserve it.
...
- **FR-008b (Staleness-probe cost)**: The per-query staleness probe MUST be a cheap
  ...
  the `embedding_model`/`embedding_dims` scalars AND the monotonic
  `vectors_write_version` counter from `project_metadata` (§D6a; no new column, no
  full scan; the counter is bumped on every `node_vectors` mutation so same-count
  in-place re-embeds and 1-for-1 renames also invalidate — added post-review,
  commit 3f6ec9e) — and it runs **outside** the gated fusion-compute leg.
...
- **Vector matrix cache**: A lazily built in-memory single-precision matrix of all matching-model vectors, carrying per-row kind and language for pre-filtering, invalidated by the FR-008b staleness probe (matching-model vector count + the `embedding_model`/`embedding_dims` scalars + the `vectors_write_version` counter; there is no `data_version` column).
...
- SPEC-001/002 already persist an embedding vector for every declaration symbol, tagged with its model and dimensionality (the `node_vectors` columns are `node_id, model, dims, vector, input_hash` — there is no `data_version` column). The FR-008b staleness probe is therefore derived from the matching-model vector count (`getEmbeddingCoverage`) plus the `embedding_model`/`embedding_dims` scalars and the monotonic `vectors_write_version` counter in `project_metadata` (bumped on every vector write so same-count re-embeds invalidate), not a `data_version` value.
