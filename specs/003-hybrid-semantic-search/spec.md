...
- **Index staleness mid-session**: A cheap per-query staleness probe (matching-model vector count from `getEmbeddingCoverage` + the `embedding_model`/`embedding_dims` scalars from `project_metadata` — FR-008b; there is no `data_version` column) invalidates and rebuilds the in-memory matrix cache when the index changed.
...
- **FR-009**: The KNN scan MUST use a lazily built in-memory single-precision matrix cache covering all vectors whose model matches the active provider's model, invalidated per query by a cheap staleness probe (matching-model vector count + the `embedding_model`/`embedding_dims` scalars — FR-008b; there is no `data_version` column); resident memory equals count×dims×4 bytes and MUST be documented (Q6).
...
- **Vector matrix cache**: A lazily built in-memory single-precision matrix of all matching-model vectors, carrying per-row kind and language for pre-filtering, invalidated by the FR-008b staleness probe (matching-model vector count + the `embedding_model`/`embedding_dims` scalars; there is no `data_version` column).
