...
- **Decision**: Implement the probe as **(vector count for the active model) + (a version anchor from `project_metadata`) + (a monotonic write-version counter)**. Concretely: `getEmbeddingCoverage(activeModel)` (`queries.ts:2489`) already returns `{ embeddable, embedded }` via matching-model JOIN counts — `embedded` is the matching-model vector count. `project_metadata` (`schema.sql:202`, `key/value/updated_at`) holds `embedding_model` + `embedding_dims` scalars. The probe compares `(embedded_count, embedding_model, embedding_dims, vectors_write_version)` — cheap (a JOINed COUNT plus three single-row metadata reads), no new column, no schema write (dormancy preserved). **Write-version (added under review item 6):** `count + model + dims` alone are BLIND to two matrix-affecting mutations that leave the count unchanged — a same-count in-place re-embed (`upsertNodeVector` ON CONFLICT DO UPDATE) and a 1-for-1 rename (delete A + insert B). The `vectors_write_version` metadata scalar, bumped on EVERY `node_vectors` mutation (upsert + orphan-delete paths in `queries.ts`), closes that gap: any such churn moves the token so the resident matrix rebuilds. It is absent on a pre-existing corpus (⇒ `0`, degrading to count/model/dims-only behavior, no migration), and — because it is written only on the vector-write path — a dormant project never creates it, so byte-parity is untouched.
- **Rationale**: Honors Constitution VII (no new column / no schema change — `vectors_write_version` is a `project_metadata` row like `embedding_model`, written only when embedding is active) and III (no upstream schema churn). The spec's intent — "detect the index changed and rebuild" — is fully met by count+metadata+write-version, now including same-count re-embeds and renames.
