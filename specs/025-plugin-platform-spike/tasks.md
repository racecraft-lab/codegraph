...
**Prerequisites**: plan.md (validation protocol V1–V19 + 12-section decision-doc blueprint + 3-day timebox map), spec.md (US1–US5, FR-001…FR-022, SC-001…SC-008), research.md (public citation inventory + OQ-8 prior-art hypothesis)
...
- [X] T009 [US1] Verify **no committed citation** in doc sections 3–4 references a private or vault path — all resolve to the enumerated public sources (join task; depends on T007+T008). **Evidence**: citation audit recorded clean (FR-004, US1 AS3, SC-002).
...
- [X] T023 [US5] Write the **skill-authoring grounding + shipped-artifact plan** (V16) in doc section 9. **First**, open the section with the **skill-authoring grounding block** (FR-022, roadmap SPEC-025 scope bullet 2) — a dedicated, citation-backed block grounding the plan + exemplar in both vendors' PUBLIC guidance: the shared agent-skills open standard (`SKILL.md` + optional `scripts/`/`references/`/`assets/`; the per-host divergences the US1 audit enumerates — discovery dirs, tool-permission frontmatter, auto-invoke opt-outs, invocation syntax); Anthropic's progressive disclosure / MCP-enhancement "recipes over the kitchen" category / what-when trigger discipline / kebab-case + exact-`SKILL.md` structural rules / no-XML + reserved-name security restrictions / optional `allowed-tools` field (pre-approval, not restriction) / the skill-to-MCP dependency mechanism as an **audit output** (qualified `ServerName:tool_name` body references per Anthropic best-practices — correct the roadmap's `metadata.mcp-server` to the hands-on evidence, never assert it unverified, per FR-002); OpenAI's `.agents/skills` scan order / explicit `$skill-name` vs implicit invocation / `agents/openai.yaml` sidecar / authoring best practices; and the vendors' published skill success criteria — each citing a public source per FR-004 / `research.md` C6. **Then** enumerate the candidate skill/agent set applying the three-leg inclusion criterion (rides `codegraph_explore` the agent already calls; adds guidance not in `server-instructions.ts` #529; expected to clear the FR-015 bar — workflows failing any leg recorded as considered-and-excluded with the reason); per-artifact **tier decision** (default FULLY OPEN for workflow/authoring skills retaining Edit/Write; built-in-only `disallowed-tools` denylists for read-only/review artifacts; never deny or re-expose the codegraph MCP tools — operator-controlled server-side; `context: fork` to a restricted subagent where a constraint must outlast the turn); the trigger surface each targets; the **agent class evaluated separately** (recording whether any agent qualifies for v1; `retrieval-guardian` excluded — reviews CodeGraph's own source, inapplicable to a user repo); the **FR-015 A/B bar definition** (artifact-off vs artifact-on: baseline = plugin MCP server with the candidate absent, treatment = same server + artifact loaded; both arms `--model sonnet --effort high`, ≥2 runs/arm, wall-clock + tool-call + Read/Grep + control repo, **plus the vendors' published skill success criteria recorded alongside** — trigger rate on relevant queries, workflow tool-call count, zero failed tool calls, with/without comparison — per the roadmap's binding SPEC-025 gate; pass = no regression); and the **FR-016 reference-not-restate line item** per candidate. **Evidence**: skill-authoring grounding block closes roadmap scope bullet 2 with public citations + all named elements present; each candidate has a tier + a validation bar (incl. the published-criteria leg); excluded workflows recorded with reasons (FR-014, FR-015, FR-016, FR-022).
- [X] T024 [US5] Draft the **ONE explore-flow exemplar skill body** (V17) in doc section 10 (the appendix) — a fully-drafted `SKILL.md`-shaped artifact that **references, never restates** `server-instructions.ts` (#529) and encodes the explore-flow retrieval recipe over `codegraph_explore`. **Non-goal guard (Q4/Q5)**: exactly one exemplar; draft NO other candidate artifact body. **Evidence**: exactly one exemplar present, no second body (FR-017, FR-016, SC-005).
...
**Non-deferrable core (must land in-timebox)**: T010, T011 (macOS launcher both hosts), T016 (Claude lever), T017 (Codex pinned-version hook test), T018 (4-step near-duplicate evasion scenario — MUST-observe per FR-011, feeds non-deferrable V14/T020), T019 (8-cell matrix), T020 (coexistence both directions), T023 (skill-authoring grounding + artifact plan), T024 (the one exemplar).
