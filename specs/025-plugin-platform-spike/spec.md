...
- **FR-011**: The document MUST specify detection and dedupe behavior in BOTH directions (plugin-detects-installer and installer-detects-plugin) so that no duplicate MCP registration and no double hook injection occur. Detection and dedupe rules MUST distinguish entries the installer writes today from entries that would be new SPEC-026 installer capability.
