...
- **Large-repo characterization**: Deterministic Louvain runs over V = indexed file count, E = distinct cross-file pairs; O(E·passes) with passes bounded. The file-graph build iterates cross-file calls/imports once (O(E_graph)), aggregates into a file-pair weight map (memory O(distinct file pairs)), self-loops dropped. Record V/E and analysis wall-clock as SC-006 evidence on ≥1 large repo (~10k+ files), not only the ~536-file fixture.
...
- **Decision**: A cluster's `id` is an **opaque token minted on first appearance** (not a rowid/positional index). The token is a **deterministic content hash of the cluster's sorted member file paths**, so an unchanged graph analyzed from scratch in any environment or clone mints identical identifiers (cross-run/cross-clone reproducibility, SC-004/FR-013). Across re-index, prior membership is read from the **last successfully-committed clusters catalog (pre-swap read)**; new clusters greedily inherit a prior id when membership **Jaccard ≥ 0.5**, one-to-one, regardless of content hash; a prior cluster that splits transfers its id to only the **single best-matching descendant**, others mint new ids; ties break deterministically on stable ordering. First successful run with no prior catalog mints all-new ids.
