...
5. **Given** both catalogs enabled and a re-index where one catalog's analysis succeeds while the other's fails, **When** the operation finishes, **Then** each catalog's state resolves independently — the succeeding catalog reads fresh (available) and the failing one retains its prior data marked stale (or unavailable on first run) — and neither outcome blocks or taints the other.
...
- **One catalog succeeds, the other fails**: When both are enabled and one run computes one successfully but the other's analysis fails, each catalog's state resolves INDEPENDENTLY — the succeeding one swaps fresh (available); the failing one independently retains its prior rows stale (or unavailable on first run). One catalog's failure never blocks, discards, or staleness-taints the other.
- **Aborted analysis**: If index/sync is cancelled before catalog analysis commits, analysis performs no partial write and the prior catalog is untouched — a cancellation is a catalog no-op, not a stale-marking failure (if the abort lands after graph_write_version advanced but before the swap, the retained prior derives as stale per FR-022).
...
- **FR-005**: System MUST bound every flow trace with fixed caps of 12 hops of depth, 20 outgoing edges per step, and 200 unique steps per flow. Per-flow work is capped (12/20/200); the NUMBER of flows is bounded only by the repo's static entry-point count — no total-flow ceiling is imposed (ratified per-flow caps; Constitution II). Aggregate flow-analysis cost O(entry_points × per-flow cap) is captured by the SC-006 large-repo measurement.
...
- **FR-008a**: Flow tracing MUST be deterministic: at each step, candidate out-edges MUST be visited in a stable total order (target file path, then edge-kind rank, then callee qualified name, then a stable edge key) BEFORE the 20-edge width cap and 200-step total cap select survivors; a node reached via multiple parents records the parent of its first visit under that order. This makes a bounded/truncated flow byte-identical across runs of an unchanged graph (SC-004) — the flow analogue of FR-013's stable cluster ordering.
- **FR-009**: Every persisted flow step MUST carry the provenance of the call edge that produced it. The root step (depth 0) has no incoming call edge and carries no provenance (null); provenance is required on every non-root step.
...
- **FR-019**: System MAY store an optional LLM-generated display label as separate, presentation-only metadata that MUST NOT affect cluster membership, identity, or canonical labels; when no LLM is configured, System MUST make no model call and MUST behave identically to the LLM-absent case. When an LLM endpoint IS configured, a failed/timed-out label call MUST be swallowed as advisory — the cluster stores its deterministic canonical label with a null display label; the failure MUST NOT fail analysis, mark the catalog stale/unavailable, or alter membership/identity/canonical labels (Constitution V). Any surfaced diagnostic MUST NOT include the endpoint URL or API key/credential — only the error class/name (mirroring the embedding pass's endpoint/key redaction).

**Lifecycle**

- **FR-020**: System MUST fully recompute both catalogs after every successful index and after every successful sync (no incremental catalog maintenance). A partial index (index_state='partial') counts as successful for catalog-analysis purposes — analysis runs and graph_write_version advances, consistent with the advisory embedding pass; the catalog is computed over the committed graph. Catalog analysis assumes single-writer serialization of a project's index/sync (the existing project index lock): analyses MUST NOT interleave such that one's pre-swap prior-membership read (FR-017a) precedes another's swap; where concurrent index/sync is possible, analysis serializes on that lock.
...
- **FR-021a**: Any catalog read that composes more than one SQL statement — notably a `total` count alongside a paged slice (FR-027/FR-028), the codegraph_get_flow detail read (a flow's header row together with its flow_steps rows), and the read-state probe (a catalog_meta row together with the content-row count that distinguishes available from available-but-empty) — MUST derive its rows from a single consistent snapshot (a single query or full-fetch-then-slice per the existing read-ops precedent, or a read transaction wrapping the statements), so a concurrent atomic swap cannot yield a torn cross-generation response. Reads run in autocommit by default (each *statement* gets its own snapshot), so multi-statement composition is where torn reads can arise; where WAL is unavailable (some virtualized/network mounts silently retain the prior journal mode) the store degrades to writer-blocks-reader, preserving the same all-or-nothing guarantee. This single-snapshot guarantee holds on BOTH read surfaces: MCP tools run inside the per-project daemon and read on its worker connections (WAL cross-connection snapshot isolation); the REST endpoints hold NO database connection of their own — the web `serve` process is a daemon *client* (`src/server/daemon-client.ts`, SPEC-005 FR-002) that forwards each catalog read to that same daemon over its socket, mirroring the existing `readNode`/`readSearch` read-forwarding, so both surfaces execute the composite read on the daemon's worker connections (one topology, not two). Either way, every catalog composite read MUST derive its rows from a single statement or a single full-fetch-then-slice, never two separately-issued statements. That single-fetch rule (not a read transaction) is the connection-topology-independent mechanism.
...
- **FR-025**: A project that has not opted in MUST run no catalog analysis and write no catalog rows and no catalog metadata; behavior is byte-identical to the pre-feature state with respect to catalog *data*. Empty catalog table definitions MAY exist in the shared schema (as `node_vectors` does today) and are not considered added cost. Read-state resolution MUST consult the live per-catalog opt-in flag FIRST: a catalog previously enabled and computed but now disabled MUST read as disabled (its retained rows/metadata are inert), never available or stale.
...
- **FR-028a**: MCP item objects and REST response bodies MUST be produced from a single shared TypeScript wire-shape definition (the catalog-store read return types), and a cross-surface parity test MUST assert the MCP item field names and the openapi FlowSummary/FlowDetail/FlowStep/ClusterSummary schemas are field-for-field identical (makes SC-009 checkable, not merely asserted).
...
- **FR-030**: For expected conditions — project not indexed, analysis disabled, catalog stale, catalog unavailable (first-run analysis failure), available-but-empty, or unknown flow/cluster identifier — every surface MUST return success-shaped guidance and MUST NOT return an error-shaped (`isError`) response. `isError` is reserved for genuine malfunctions (corrupt/unreadable catalog, internal error) and security refusals (Constitution VI). Each surface result MUST carry a machine-readable `state` field spanning available|stale|empty|unavailable|disabled|not_indexed so consumers branch structurally; an empty items array MUST NOT be the sole signal of the unavailable state.
...
- **SC-001**: In an opted-in repository, every detected entry point yields exactly one flow, and 100% of persisted non-root flow steps carry a provenance value (the root, having no incoming edge, carries none).
...
- **SC-007**: A project that has not opted in adds zero measurable analysis overhead and writes no catalog rows or metadata — behavior is byte-identical to the pre-feature state with respect to catalog data (empty table definitions in the shared schema, as with `node_vectors`, are not counted). "Zero measurable overhead" means (a) zero catalog rows/metadata written (asserted count=0) and (b) full-index median wall-clock within the run-to-run variance band (≤2%) of a pre-feature build under the same paired harness — not a literal-zero timing assertion.
...
- **SC-009**: The MCP and REST surfaces return the same field semantics for the same catalog data, and expected conditions (not indexed, disabled, stale, unavailable, available-but-empty, unknown identifier) return success-shaped guidance in 100% of cases — never an error-shaped response.
- **SC-010**: The self-repo dogfood UAT confirms the `codegraph index` CLI entry point's flow reaches the extraction → resolution → (LSP) → embedding stages with correct per-step provenance and truncation state, and that this repository's source modules land in coherent clusters whose identifiers stay stable across two consecutive re-indexes. Because both catalogs fully recompute on every successful sync and the watcher drives sync() on individual settled saves (coalesced by the FileWatcher debounce, default 2000ms), the added per-sync catalog cost on the dogfood repo MUST be recorded (median added wall-clock per single-file sync) as UAT evidence alongside SC-010; watch-driven syncs must stay responsive.
- **SC-011**: With no LLM endpoint configured, catalog analysis makes zero model calls and produces byte-identical catalog output to the LLM-absent case (display labels all null) — verified by dormancy tests in 100% of runs.
- **SC-012**: `codegraph_explore` output is byte-identical for a fixed corpus of existing queries before vs after this feature (golden/snapshot test), AND `src/mcp/server-instructions.ts` and the explore handler are untouched — no added steering, no budget change (FR-031/Q16).
...
- The "fixture monorepo" used for the performance benchmark is a committed, deterministically-generated multi-language fixture at `__tests__/analysis/fixtures/benchmark-monorepo/` (≥3 languages/frameworks, including a god-function fan-out plus a route and a CLI entry point), materialized by the test harness; the identical fixture is used for both benchmark arms, with embedding and LSP configuration held constant across the paired runs.
