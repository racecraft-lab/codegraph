<!-- speckit-pro-review-packet-source: specs/001-embedding-infrastructure/.process/pr/packet-b.json -->

## Summary

<!-- speckit-pro-editable:summary:start -->
**Stacked PR 2 of 2 — review after (and merge into/after) the full-index embedding PR this branch is based on.** Completes the embedding substrate with the day-to-day layer: incremental freshness on edit (only genuinely changed symbols re-embed, per-symbol via input hashing), reconciliation of deleted symbols' vectors, late-configuration backfill (one plain sync heals a pre-configured index to 100%), and endpoint-outage resilience (clean advisory abort, automatic stateless resume). Also carries the six pre-PR review fixes for the combined feature (surfaced advisory aborts, activated request concurrency, retry reclassification of mid-body transport failures, timer-based lock refresh, zero-length vector rejection, changelog wording) plus the UAT runbook and process evidence.
...
- Incremental selection in the embed pass: missing vectors + model-switched rows + input-hash-changed symbols (a network-free compose-and-compare scan); reconciliation delete (anti-join over the complete live symbol set) runs on every vector-preserving pass — vectors survive a file's node delete/re-insert cycle by design (no cascade).
- `sync()` wiring in the same advisory slot as the full-index path — deliberately not gated on change count, so a zero-change sync heals missing vectors; the daemon file-watcher inherits the pass through the shared sync path with zero MCP-layer changes.
- Resilience pins: mid-pass outage → bounded retries → clean abort with completed batches durable → next run re-embeds only what's missing (re-selection is the resume; no checkpoint state exists anywhere); later-batch dimension conflicts abort actionably.
- Review fixes across the feature: advisory aborts now logged (a bad key or dims conflict is no longer observably identical to success), concurrency super-chunking makes the concurrency setting real, mid-body transport failures retry instead of instantly aborting, the held index lock refreshes on a 30-second timer during long passes, zero-length embeddings are rejected as malformed.
- New test suites: incremental-sync (14) and resilience (4, falsifiability-probed); full suite at 2212 passing.
- **Follow-up commits since opening:** all 8 Copilot review findings across both stacked PRs fixed here (pass-level vector-count guard, non-finite embedding rejection, mid-body transport-failure retry reclassification, pass-scoped per-file source cache, doc/wording corrections); a committed direnv loader (`.envrc` sourcing an untracked `.envrc.local`) for local endpoint dogfood config; and a new user-facing feature: `codegraph init` now adds `.codegraph/` to the project's `.gitignore` automatically (git repos only, idempotent, advisory) — the local index now carries embedding vectors and must never be committed. Each landed with its own tests; suite green throughout.
...
Without this layer the substrate is a one-shot batch job: vectors drift stale on every edit and a user who configures an endpoint after indexing has no recovery path short of a full re-index. With it, freshness is automatic at negligible endpoint cost, late configuration heals with one ordinary sync, and an endpoint outage costs nothing but deferred coverage. The advisory contract holds throughout — no embedding failure mode can fail or hang an index or sync.
