...
# session): each arm PRE-WARMS the internal persistent daemon for its target so
# claude connects to an already-bound, index-loaded daemon instantly — before
# the agent's first turn. The prewarm process lets the CLI add its required
# WASM runtime flags; only the lightweight MCP proxy skips that startup re-exec.
# Without this, on a multi-step task the agent dives into Read/grep before
# codegraph finishes startup (worse under the CPU contention of a nested run)
# and runs with NO codegraph.
#
...
#      AGENT_EVAL_DAEMON_WAIT_MS (default: 60000)
set -uo pipefail
...
DAEMON_WAIT_MS="${AGENT_EVAL_DAEMON_WAIT_MS:-60000}"
...
while IFS= read -r file; do
  CHANGED+=("$file")
done < <(git -C "$ENGINE" diff --name-only "$BASE_REF" HEAD -- src 2>/dev/null)
[ "${#CHANGED[@]}" -gt 0 ] || { echo "no src/ changes between $BASE_REF and HEAD — nothing to A/B"; exit 1; }
...
prewarm() { # target — spawn a persistent daemon (current $BIN) and wait for its socket
  ...
  CODEGRAPH_DAEMON_INTERNAL=1 CODEGRAPH_DAEMON_IDLE_TIMEOUT_MS=1800000 \
    node "$BIN" serve --mcp --path "$1" </dev/null >/dev/null 2>&1 &
  if node -e 'const fs=require("fs");const deadline=Date.now()+Number(process.argv[2]);const t=setInterval(()=>{if(fs.existsSync(process.argv[1]+"/.codegraph/daemon.sock")){clearInterval(t);process.exit(0)}if(Date.now()>deadline){clearInterval(t);process.exit(1)}},100)' "$1" "$DAEMON_WAIT_MS"; then
    echo "  daemon warm: $1"
  else
    echo "  ERROR: daemon never bound for $1 within ${DAEMON_WAIT_MS}ms"
    return 1
  fi
}
...
run_arm baseline "$OUT/t-base" || exit 1
