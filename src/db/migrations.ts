...
const migrations: Migration[] = [
  ...
  },
  {
    version: 11,
    description:
      'Add SPEC-014 CFG status, block, and edge tables with by-value function ownership and ordered-read indexes',
    up: (db) => {
      // DDL only — instant on existing databases. CFG status is keyed by public
      // function id and keeps file/language/span metadata by value, with no FK
      // to nodes(id), because sync deletes and reinserts node rows. Cascades are
      // confined to CFG-owned payload rows. Keep this definition in lockstep
      // with schema.sql.
      db.exec(`
