 ...
 * OCaml (fork, SPEC-023): tree-sitter-wasms ships neither grammar; we vendor
 * implementation (`ocaml`) and interface (`ocaml_interface`) wasms built from
 * tree-sitter-ocaml (MIT) — keyed by GrammarKey because `ocaml_interface` is
 * an internal grammar key, not a Language.
 */
const VENDORED_WASM_LANGS: ReadonlySet<GrammarKey> = new Set<GrammarKey>([
  'pascal', 'scala', 'lua', 'luau', 'csharp', 'r', 'cfml', 'cfscript', 'cfquery',
  'cobol', 'vbnet', 'erlang', 'terraform', 'arkts', 'nix', 'ocaml', 'ocaml_interface',
]);

/** Absolute path of a language's grammar WASM (vendored or tree-sitter-wasms). */
function resolveWasmPath(lang: GrammarKey): string {
...
export async function readGrammarWasmBytes(languages: Language[]): Promise<Record<string, Uint8Array>> {
  ...
  // `ocaml` is two grammars (implementation + interface) — pre-read both so
  // .mli parsing keeps the from-memory path (mirrors loadGrammarsForLanguages).
  const grammarKeys = toRead.flatMap((lang): GrammarKey[] =>
    lang === 'ocaml' ? ['ocaml', 'ocaml_interface'] : [lang]
  );
  for (const lang of grammarKeys) {
    try {
...
export async function loadGrammarsForLanguages(languages: Language[], wasmBytes?: Record<string, Uint8Array>): Promise<void> {
  ...
  for (const grammarKey of toLoad) {
    try {
      const bytes = wasmBytes?.[grammarKey];
      const language = await WasmLanguage.load(bytes ?? resolveWasmPath(grammarKey));
